...
var model_df = spark.read.schema(modelSchema).csv("/user/ak8257/Pegasus/pegasus_score/")
...
val indexer3 = new StringIndexer().setInputCol("city_type").setOutputCol("city_type_n") 
model_df = indexer3.fit(model_df).transform(model_df)
model_df = model_df.drop(col("city_type"))

val indexer4 = new StringIndexer().setInputCol("loyalty_bucket").setOutputCol("loyalty_bucket_n") 
model_df = indexer4.fit(model_df).transform(model_df)
model_df = model_df.drop(col("loyalty_bucket"))

val indexer5 = new StringIndexer().setInputCol("activity").setOutputCol("activity_n") 
model_df = indexer5.fit(model_df).transform(model_df)
model_df = model_df.drop(col("activity"))


val cols = Array("txns", "searches", "fav_city", "city_search", "ap_bucket_n", "asp_bucket_n", "city_type_n", "loyalty_bucket_n", "activity_n")
...
val indexe6r = new StringIndexer().setInputCol("pscore").setOutputCol("label")
val labelDf = indexe6r.fit(featureDf).transform(featureDf)
...
// // predict pass/fail status of sample data set

...
object Main extends App {
  def Main(args: Array[String]): Unit = {
    ...
    var windowSpec2 = Window.partitionBy("user").orderBy(col("last_txn_date").asc)
    var df_first_txn_date = df.withColumn("temp", first("last_txn_date").over(windowSpec2)).select("*").where(col("temp") === col("last_txn_date")).drop("temp")

...
val hotel_city_map = "Select HotelName,CityName, CityType, concat(CityName, '_', HotelName) as hotelid from hotel_city_map a join city_type b on a.City = b.CityName"
sqlCtx.sql(hotel_city_map).createOrReplaceTempView("hotel_search")

transaction_df.createOrReplaceTempView("transaction_data")
val transaction_data = "Select HotelName, BookingAmount/RoomNights as ASP_Amount, concat(UserName, '#', string(date(TimeStamp)), '#', string(date(CheckIn))) as super_userid, concat(CityName, '_', HotelName) as hotelid from transaction_data"
